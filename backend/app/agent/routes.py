...
async def summarize_room(room_id: str, current_user: UserInDB = Depends(get_current_user)):
...
{{
    ...
    try:
        ...
        logger.error(f"NVIDIA API timeout for room {room_id}")
        return {
            "summary": {
                "summary": "Summary generation timed out. The AI service is temporarily slow.",
                "decisions": [],
                "action_items": [],
                "open_questions": []
            },
            "markdown": "# Workspace Session Summary\n\n## Overview\nSummary generation timed out. Please try again in a moment."
        }
    except Exception as e:
        logger.error(f"NVIDIA API error for room {room_id}: {e}")
        return {
            "summary": {
